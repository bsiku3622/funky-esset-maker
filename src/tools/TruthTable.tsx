...
export default function TruthTableTool() {
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORE_KEY, JSON.stringify({ input, useTF, colorize, header, fontSize, bg }))
    } catch {
      /* ignore */
    }
  }, [input, useTF, colorize, header, fontSize, bg])
  ...
  useLayoutEffect(() => {
    recompute()
  }, [recompute, input, fontSize, bg, useTF])
  useEffect(() => {
    const stage = stageRef.current
    const shot = shotRef.current
    if (!stage || !shot) return
